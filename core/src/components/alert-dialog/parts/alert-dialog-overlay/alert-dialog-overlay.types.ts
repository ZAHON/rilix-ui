...
export interface AlertDialogOverlayProps extends PropsOf<'div'> {
  /**
   * A `QRL` callback function invoked after the alert dialog overlay's open or close animation/transition has fully completed.
  ...
  render$?: (
    ...
    state: {
       ...
       * It is `true` when the alert dialog is open, and `false` when closed.
       */
