...
describe('clamp', () => {
  ...
  it('should throw an error if min is greater than max', () => {
    expect(() => clamp({ value: 50, min: 100, max: 0 })).toThrow(
      new Error(
        "Rilix UI: Invalid range for 'clamp' utility. The 'min' param (100) must be less than or equal to the 'max' param (0)."
      )
    );
    expect(() => clamp({ value: 10, min: 20, max: -5 })).toThrow(/Rilix UI: Invalid range for 'clamp' utility/);
  });
